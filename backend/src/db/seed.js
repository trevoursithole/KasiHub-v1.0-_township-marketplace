// Must load env BEFORE database.js is required
...
async function seed() {
  ...
  const insU = db.prepare(`
    INSERT INTO users
      (id, name, phone, password_hash, section, avatar_initials,
       is_verified, verification_anchors, rating, total_sales,
       total_earnings, is_runner, runner_transport, runner_rate)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  `);
  for (const u of users) {
    insU.run(u.id, u.name, u.phone, hash, u.section, u.initials,
      u.verified, u.anchors, u.rating, u.sales, u.earned,
      u.runner, u.rt || null, u.rr || null);
  }
  ...
  const insR = db.prepare(`
    ...
    VALUES (?,?,?,?,1,?,?,?)
  `);
  const trips = [234, 89, 156, 67];
  users.filter(u => u.runner).forEach((u, i) =>
    insR.run(uuidv4(), u.id, u.rt, u.rr, u.section, trips[i] || 50, u.rating));

  /* ── Safe-Trade Zones ── */
  const insZ = db.prepare(`
    INSERT INTO safe_zones (id, name, type, section, distance_km, walk_minutes, hours, has_cctv)
    VALUES (?,?,?,?,?,?,?,?)
  `);
  [
  ...
  /* ── Listings ── */
  // SQLite-compatible datetime format: "YYYY-MM-DD HH:MM:SS"
  const flashEnd = hours => {
  ...
  const insL = db.prepare(`
    INSERT INTO listings
      (id, seller_id, title, description, price, category, condition,
       section, emoji, is_flash_sale, flash_ends_at, status)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,'active')
  `);
  ...
  const listings = [
    ...
    [uuidv4(), users[8].id, 'Office Chair — almost new',              'Bought 3 months ago. Adjustable height and armrests. Cushion still firm.',        380, 'Furniture',   'Good', 'Mamelodi West', '🪑', 0, null         ],
    [uuidv4(), users[1].id, 'Eggs x30 — farm fresh',                  'Fresh eggs collected this morning from local farm. While stocks last.',            58,  'Food',        'New',  'Block L',       '🥚', 1, flashEnd(3)  ],
    [uuidv4(), users[1].id, 'Brown Bread 4-pack',                     'Albany brown bread, best before end of week.',                                     44,  'Food',        'New',  'Block L',       '🍞', 1, flashEnd(5)  ],
    [uuidv4(), users[1].id, 'Mogodu — fresh batch',                   'Freshly prepared tripe. Sold by the portion. Limited quantity today only.',        75,  'Food',        'New',  'Block L',       '🥩', 1, flashEnd(1)  ],
    [uuidv4(), users[2].id, 'Plumbing Services — same day',           'Qualified plumber. Leaking taps, burst pipes, geyser installation. Block L.',     250, 'Services',    'New',  'Block L',       '🔧', 0, null         ],
    [uuidv4(), users[2].id, 'Grade 12 Maths Tutoring',                'Help with algebra, calculus, and past papers. R80/hr. Ext 4 & Block L.',           80,  'Education',   'New',  'Ext 4',         '📚', 0, null         ],
    [uuidv4(), users[8].id, 'Hair braiding — all styles',             'Box braids, cornrows, loc styles. Home-based. Clean setup. Book in advance.',     180, 'Beauty',      'New',  'Mamelodi West', '💅', 0, null         ],
    [uuidv4(), users[3].id, 'Adidas tracksuit top — size M',          'Genuine Adidas from Totalsports. Navy blue. Worn twice only.',                    150, 'Fashion',     'Good', 'Block L',       '👕', 0, null         ],
    [uuidv4(), users[1].id, 'Mageu 2L — homemade',                    'Traditional mageu, made fresh this morning. Smooth texture.',                      22,  'Food',        'New',  'Block L',       '🫙', 1, flashEnd(6)  ],
  ];
  listings.forEach(l => insL.run(...l));

  /* ── One completed transaction (demo receipt) ── */
  const txId = uuidv4();
  db.prepare(`
    INSERT INTO transactions
      (id, reference, buyer_id, seller_id, listing_id,
       item_price, runner_fee, total, delivery_method, status, qr_scanned, completed_at)
    VALUES (?,?,?,?,?, 45,15,60, 'runner','completed',1, datetime('now','-2 hours'))
  `).run(txId, 'KH-2025-08741', users[0].id, users[1].id, listings[0][0]);

  /* ── Notifications for Thandeka ── */
  const insN = db.prepare(`
    INSERT INTO notifications (id, user_id, type, title, body, icon, is_read, link_screen)
    VALUES (?,?,?,?,?,?,?,?)
  `);
  [
    ['runner_update', 'Runner on the way!',          'Zakhele picked up your Kota from Mama Dlamini. ETA 8 minutes.',     '🚴', 0, 'transactions'],
    ['message',       'New message from Sipho',      '"Is the Samsung still available? Can you do R800?"',                 '💬', 0, 'messages'    ],
    ['review',        'New 5-star review!',           'Mama Dlamini: "Quick payment, no hassle. Will sell again!"',        '⭐', 1, 'profile'     ],
    ['flash_sale',    'Flash sale ending soon',       'Mogodu in Block L — under 1 hour left. R75 only.',                  '⚡', 1, 'home'        ],
    ['verification',  'Verified Resident confirmed',  'Your badge is now active after 3 community vouches in Block L.',   '✅', 1, 'profile'     ],
  ].forEach(([type, title, body, icon, read, screen]) =>
    insN.run(uuidv4(), users[0].id, type, title, body, icon, read, screen));

  console.log(`✅  Seeded:`);
  console.log(`    ${users.length} users  ·  ${listings.length} listings  ·  5 safe zones  ·  4 runners`);
  console.log(`    1 demo transaction  ·  5 notifications\n`);
  process.exit(0);
}
